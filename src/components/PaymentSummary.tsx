...
import { PLATFORM_COMMISSION, IVA_RATE } from '../data/standardRates';
          ...
          {wantsInvoice ? (
            <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 space-y-3">
              ...
              <div className="bg-white rounded-lg p-3 space-y-1 text-[11px] border border-indigo-100">
                <div className="flex justify-between"><span className="text-slate-600">Servicio de enfermería:</span><span className="font-bold text-slate-700">${(totalPrice - managementFee).toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Gestión fiscal + IVA:</span><span className="font-bold text-slate-600">+${managementFee.toFixed(2)}</span></div>
              ...
              <p className="text-[10px] text-indigo-600 leading-relaxed pl-6">
                Transfiere el total (${totalPrice.toFixed(2)}) a la cuenta arriba. BienCuidar emite la factura o crédito fiscal correspondiente.
              </p>
