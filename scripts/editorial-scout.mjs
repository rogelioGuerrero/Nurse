...
// ── Leer artículos ya publicados desde Supabase (memory del pipeline) ──
async function getPublishedArticles() {
  // Primero intentar Supabase
  if (SUPABASE_ANON_KEY) {
    try {
      const url = `${SUPABASE_URL}/rest/v1/content_history?select=topic,status,series_position,published_at,teaser,next_topic&order=series_position.asc`;
      ...
      if (res.ok) {
        ...
        if (rows.length > 0) {
          return rows.map((r) => {
            const status = r.status === "published" ? " ✅" : r.status === "generated" ? " (generado)" : "";
            return `${r.series_position || "?"}. ${r.topic}${status}`;
          });
        }
      }
    } catch (e) {
      console.log("  (Supabase no disponible, fallback a archivos locales)");
    }
  }
...
async function digestAll(scanResults) {
  console.log("\n═══════════════════════════════════════════════════");
  console.log("FASE 2: DIGEST — Destilando cada dominio (GPT-OSS 120B)");
  console.log("═══════════════════════════════════════════════════\n");
...
async function proposeEditorialLines(digests) {
  ...
  // Leer insights de Facebook si existen
  let fbInsights = "";
  try {
    const insightsPath = resolve(dirname(fileURLToPath(import.meta.url)), "fb-insights-summary.txt");
    if (existsSync(insightsPath)) {
      fbInsights = readFileSync(insightsPath, "utf-8");
      console.log(`FB insights: ${fbInsights.split("\n").filter(l => l.startsWith("#")).length} líneas leídas`);
    }
  } catch {}
  console.log("");
...
${fbInsights || "(no disponibles — ejecutar node scripts/fb-insights.mjs antes del scout)"}
...
# Fases: SCAN (Compound) → DIGEST (GPT-OSS 120B) → PROPOSE (GPT-OSS 120B)
#
...
Artículos publicados previos: ${published.length}
`;
