...
async function main() {
  ...
  // 4. Actualizar content_history en Supabase
  try {
    const updateRes = await fetch(`${SUPABASE_URL}/rest/v1/content_history?topic=ilike.${encodeURIComponent(message.slice(0, 50))}&order=id.desc&limit=1`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
    ...
    // No crítico — el post ya se publicó
  }
}
